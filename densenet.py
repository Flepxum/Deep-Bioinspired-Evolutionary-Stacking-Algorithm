...
from typing import Any, Callable, Dict, List, Mapping, Optional, TypeVar, Tuple, Type
...
class WeightsEnum(StrEnum):
    ...
    def get_state_dict(self, progress: bool, n: int) -> Mapping[str, Any]:
...
def _load_state_dict(model: nn.Module, weights: WeightsEnum, progress: bool, n: int) -> None:
    ...
    state_dict = weights.get_state_dict(progress=progress, n=n)
...
def _densenet(
    ...
    if weights is not None:
        _load_state_dict(model=model, weights=weights, progress=progress, n=n)
...
def dcanet121(*, weights: Optional[DenseNet121_Weights] = None, progress: bool = True, n: int,
              **kwargs: Any) -> DenseNet:
    weights = DenseNet121_Weights.verify(weights)
    if weights == DenseNet121_Weights.IMAGENET1K_V1:
        return _densenet(32, (6, 12, 24, 16), 64, weights, n, progress, **kwargs)
    elif weights == DenseNet121_Weights.IMAGENET1K_V2:
        return _densenet(48, (6, 12, 36, 24), 96, weights, n, progress, **kwargs)
    else:
        return _densenet(32, (6, 12, 32, 32), 64, weights, n, progress, **kwargs)
